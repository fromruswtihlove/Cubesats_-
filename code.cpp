...
float lut_pan_x[] = { -58, -50, -42, -32, -20,  -5,   7,  16,  24 }; // Основное движение Pan
float lut_tilt_x[] = { 8,   8,   8,   8,   8,   9,  10,  10,  10 }; // Влияние X на Tilt (паразитное)
float lut_tilt_y[] = { -33, -25, -16,  -5,   8,  20,  32,  42,  51 }; // Основное движение Tilt
float lut_pan_y[] = { -17, -17, -17, -17, -20, -20, -20, -20, -20 }; // Влияние Y на Pan (паразитное/люфт)
...
float getInterpolatedValue(float val, float* table) {
    ...
    int i = (int)indexFloat;       // Целая часть индекса
    float frac = indexFloat - i;   // Дробная часть (вес следующей точки)

    // Если мы на последней точке, просто возвращаем её
    if (i >= 8) return table[8];

    // Линейная интерполяция: Val = A + (B-A)*frac
...
void moveTo(float inputX, float inputY) {
    ...
    float pan_center = lut_pan_x[4];   // Значение при X=0 (-20)
    float tilt_center = lut_tilt_y[4]; // Значение при Y=0 (8)
...
void runMacro(int m) {
    setLaser(1);
    if (m == 5) { runMacro(1); runMacro(2); runMacro(3); runMacro(4); }
    else if (m == 1) { for (int i = -40; i <= 40; i += 10) { moveTo(i, 0); delay(400); } }
    else if (m == 2) { for (int i = -40; i <= 40; i += 10) { moveTo(0, i); delay(400); } }
    else if (m == 3) { for (int i = -40; i <= 40; i += 10) { moveTo(i, i); delay(400); } }
    else if (m == 4) { for (int i = -40; i <= 40; i += 10) { moveTo(i, -i); delay(400); } }
